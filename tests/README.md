...
1. Create an empty database such as `qrhunt_test` and configure the WordPress test library's `wp-tests-config.php` to use it.
2. In the Local Site Shell, set the following environment variables:

```powershell
$env:WP_TESTS_DIR = 'D:\path\to\wordpress-tests-lib'
$env:QRHUNT_TEST_DB = 'qrhunt_test'
$env:QRHUNT_TESTS_ALLOW_DB_RESET = '1'
```
